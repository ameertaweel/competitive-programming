...
int main() {
	...
	while (t--) {
		...
		for (char i = 'a'; i <= 'z'; i++) replace[i] = i;

		char max_reduced = 'a';

		for (char c : s) {
			if (c > max_reduced && k > 0) {
				il need = c - max_reduced;
				il reduce = min(need, k);
				char replace_c = replace[c - reduce];
				for (char i = c; i > c - reduce; i--) replace[i] = replace_c;
				k -= reduce;
				max_reduced = c;
			}
			cout << replace[c];
