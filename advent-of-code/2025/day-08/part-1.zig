...
pub fn UnionFind(comptime MAX_SIZE: usize) type {
    return struct {
        ...
        pub fn merge(self: *@This(), a: usize, b: usize) void {
...
pub fn main() !void {
    ...
    var sizes: [MAX_POINTS]usize = undefined;
    ...
    for (0..@min(LINKS_COUNT, distances_len)) |i| {
        const d = distances[i];
        uf.merge(d.a, d.b);
    }

    std.mem.copyForwards(usize, sizes[0..points_len], uf.size[0..points_len]);
    std.mem.sort(usize, sizes[0..points_len], {}, comptime std.sort.desc(usize));

    const ans = sizes[0] * sizes[1] * sizes[2];
    print("{d}\n", .{ans});
}
