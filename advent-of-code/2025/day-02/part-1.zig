...
pub fn main() !void {
    ...
    while (try reader.interface.takeDelimiter(',')) |line| {
        ...
        const pattern_beg = getPatternCeil(u64, beg, 2);
        const pattern_end = getPatternFloor(u64, end, 2);

        // DEBUGGING
        // print("(PATTERN_BEG, PATTERN_END) = ({d}, {d})\n", .{ pattern_beg, pattern_end });

        const invalid_beg = repeatPattern(u64, pattern_beg, 2);
        const invalid_end = repeatPattern(u64, pattern_end, 2);
        ...
        if (invalid_beg <= invalid_end) {
            for (pattern_beg..pattern_end + 1) |pattern| {
                ans += repeatPattern(u64, pattern, 2);
            }
        } else if (invalid_beg <= end) {
            ans += invalid_beg;
        } else if (invalid_end >= beg) {
            ans += invalid_end;
        }
    }
...
pub fn repeatPattern(comptime T: type, pattern: T, times: usize) T {
    ...
    for (1..times) |_| {
        repeated = repeated * pow10(T, pattern_d) + repeated;
    }
    return repeated;
}
