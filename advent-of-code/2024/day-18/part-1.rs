...
const BYTES_TO_SIMULATE: i64 = 1024;
...
fn main() {
    ...
    for (row, col) in bytes.iter().take(BYTES_TO_SIMULATE as usize) {
        is_corrupt[*row as usize][*col as usize] = true;
    }
    let is_corrupt = is_corrupt;

    let mut stack: VecDeque<(i64, i64, i64)> = VecDeque::new();
    stack.push_back((0, 0, 0));

    let mut min_cost: Vec<Vec<i64>> = vec![vec![GRID_SIZE * GRID_SIZE; GRID_SIZE as usize]; GRID_SIZE as usize];
    ...
    println!("{}", min_cost[(GRID_SIZE - 1) as usize][(GRID_SIZE - 1) as usize]);
}
