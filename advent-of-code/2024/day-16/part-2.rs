...
fn main() {
    ...
    let mut min_cost_spot: HashSet<(i64, i64)> = HashSet::new();

    // (Row, Col), Direction, Cost So Far
    let mut q: VecDeque<((i64, i64), Direction, i64, HashSet<(i64, i64)>)> = VecDeque::new();
    q.push_back((pos_beg, Direction::East, 0, HashSet::new()));

    // ((Row, Col), Direction) -> Minimum Cost So Far
    let mut min_cost_state: HashMap<((i64, i64), Direction), i64> = HashMap::new();

    while q.len() > 0 {
        let ((row, col), dir, cost, mut seen) = q.pop_front().unwrap();
        let pos = (row, col);
        ...
        if pos == pos_end {
            if cost < min_cost_path {
                min_cost_path = cost;
                min_cost_spot = seen;
            } else if cost == min_cost_path {
                min_cost_spot.extend(seen);
            }
            continue;
        }
        ...
        let d = dir.clone();
        q.push_back((d.apply(&pos), d, cost + COST_MOVE, seen.clone()));
        let d = dir.turn_cw();
        q.push_back((d.apply(&pos), d, cost + COST_MOVE + COST_TURN, seen.clone()));
        let d = dir.turn_ccw();
        q.push_back((d.apply(&pos), d, cost + COST_MOVE + COST_TURN, seen.clone()));
    }

    println!("{}", min_cost_spot.len());
}
